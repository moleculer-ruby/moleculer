  ...
  module Packets
    ...
    class Info < Base
      ...
      class Client
        ...
        def initialize(data)
          @type         = Hash.fetch(data, :type, nil)
          @version      = Hash.fetch(data, :version, nil)
          @lang_version = Hash.fetch(data, :lang_version, nil)
        end
      ...
      def initialize(config, data = {})
        super(config, data)
        @services = Hash.fetch(data, :services)
        @ip_list  = Hash.fetch(data, :ip_list)
        @hostname = Hash.fetch(data, :hostname)
        @client   = Client.new(Hash.fetch(data, :client, {}))
        node      = Hash.fetch(data, :node, nil)
        @node_id  = Hash.fetch(data, :node_id, node&.id)
      end
