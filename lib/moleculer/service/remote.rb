module Moleculer
  ...
  module Service
    ...
    def self.from_remote_info(service_info, service_node)
      Class.new(Remote) do
        service_name Support::Support::Hash.fetch(service_info, :name)
    ...
    class Remote < Base
      class << self
        ...
        def fetch_actions(service_info)
          seq = 0
          Support::Support::Hash.fetch(service_info, :actions).values.each do |a|
            next if Support::Support::Hash.fetch(a, :name) =~ /^\$/
            ...
            action(Support::Support::Hash.fetch(a, :name), "action_#{seq}".to_sym)
            seq += 1
          end
        end
        ...
        def fetch_events(service_info)
          seq = 0
          Support::Support::Hash.fetch(service_info, :events).values.each do |a|
            name = Support::Support::Hash.fetch(a, :name)
